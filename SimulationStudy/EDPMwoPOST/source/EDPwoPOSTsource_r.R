...
update_reg_con = function(Y_temp,matX_temp,Z_temp,beta_curr,
...
EDPmediationMCMC = function(Y, M, Z, C, 
  ...
  # Define the number of mediator variables
  p_M = length(M)/N
  ...
  B_Mbeta = var_Mbeta * INVmatXtX
  c_Mbeta = N/5
  B_Mbeta = c_Mbeta * B_Mbeta
  Binv_Mbeta = matXtX/(var_Mbeta * c_Mbeta)
  aBinv_Mbeta = Binv_Mbeta %*% a_Mbeta
  aaBinv_Mbeta = c(t(a_Mbeta) %*% aBinv_Mbeta)
  ...
  aaBinv_Ybeta = c(t(a_Ybeta) %*% aBinv_Ybeta)
  ...
  Ky = 2
  Sy = kmeans(cbind(Y, M, Z, scaC), Ky)$cluster
  Syx[,1] = Sy
  ...
  for (k in 1:Ky) {
    ind_Sy = (Sy == k)
    Sx[ind_Sy] = kmeans(X[ind_Sy,], Kx)$cluster
  }
  Syx[,2] = Sx
  ...
  # Update values of Mbeta in mediator regressions  
  # Update values of Ybeta in outcome regressions
  ...
  for (k in unique_Sy) {
    ...
    Mregression_list = update_reg_con(M[ind_Sy],matX[ind_Sy,],Z[ind_Sy],MbetaPars[,k],
                                       a_Msig2,b_Msig2,Binv_Mbeta,aBinv_Mbeta)
    Yregression_list = update_reg_con(Y[ind_Sy],matM[ind_Sy,],Z[ind_Sy],YbetaPars[,k],
  ...
  for (num in 1:num_MC_prior) {
    ...
    matVMbeta_temp = matX %*% Mbeta_temp
  ...
  for (gibbs_reps in 1:gibbs_total) {
    ...
    cluster_res = EDPcluster_concon(N, py, pm, p_Z, p_C1, p_C2, 
                                    Y, M, X, matM, matX,
                                    Sy, Sx, unique_Syx,
                                    YbetaPars, Ysig2Pars, MbetaPars, Msig2Pars,
                                    ZpiPars, XpiPars, XmuPars, Xtau2Pars,
                                    alpha_theta, alpha_omega,
                                    f0_y, f0_m, f0_x,
                                    a_Ysig2, b_Ysig2, a_Ybeta, B_Ybeta, 
                                    Binv_Ybeta, aBinv_Ybeta, aaBinv_Ybeta,
                                    a_Msig2, b_Msig2, a_Mbeta, B_Mbeta, 
                                    Binv_Mbeta, aBinv_Mbeta, aaBinv_Mbeta,
    ...
    # Vectors of parameters (XpiPars, Xtau2Pars, XmuPars) is for cluster function
