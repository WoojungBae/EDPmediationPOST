...
List EDPcluster_concon(int const& N, int const& p_Z, int const& p_C1, int const& p_C2,
  ...
  double Ylikreg, Mlikreg, Vlik, Xlik;
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int k = 0; k < Ky; k++) {
      ...
      for (int r = 0; r < Kx_Sy; r++) {
        ...
        Vlik = normpdf(V_temp, dot(matX_temp, VbetaPars.col(count)), sqrt(Vsig2Pars(count)));
        ...
        probs(count) = (n_k_woi * (n_rk_woi/(n_k_woi+alpha_omega))) * Ylikreg * Mlikreg * Vlik * Xlik;
...
List EDPpostmc_concon(int const& p_C1, int const& p_C2,
  ...
  // return List::create(_["NIE"] = E_Y_z0m0,
  //                     _["NDE"] = E_Y_z1mr,
  //                     _["ATE"] = E_Y_z1m1);
}
