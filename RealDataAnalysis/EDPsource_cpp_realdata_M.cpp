...
List EDPcluster_concon(int const& N, int const& p_Z, int const& p_C1, int const& p_C2,
                       vec const& M, vec const& V, mat const& X,
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int k = 0; k < Ky; k++) {
      ...
      // ---------------------------------------------------------------------------------------
      probs(Kyx+k) = (n_k_woi * (alpha_omega/(n_k_woi+alpha_omega))) * Mlikreg * f0_v(i) * f0_x(i);
    }
    
    // ---------------------------------------------------------------------------------------
    // ---------------------------------- New y&m-x clusters ---------------------------------
    // ---------------------------------------------------------------------------------------
    probs(Kyx+Ky) = alpha_theta * f0_m(i) * f0_v(i) * f0_x(i);
...
// [[Rcpp::export]]
List EDPpostmc_M(int const& p_C1, int const& p_C2,
                      vec const& n_k, mat const& n_rk, 
                      int const& Ky, int const& Kyx, vec const& max_Kx_Sy, 
                      int const& D, int const& num_MC, vec const& rho_mc,
  ...
  for (int k = 0; k < Ky_mc; k++) {
    ...
    for (int r = 0; r < max_Kx_Sy_temp; r++) {
      ...
      for (int q = 0; q < p_Z; q++) {
        Zpi_prop = Zpi_mc(q, r, k);
        for (int d = 0; d < num_Syx_mc; d++) {
          Z_mc(ind_Syx_mc(d)) = R::rbinom(1, Zpi_prop);
        }
      }
      for (int q = 0; q < p_C1; q++) {
        Xpi_prop = Xpi_mc(q, r, k);
  ...
  // Result is vector of size D
  vec f0_c_mc = prod(f0_c_all_mc, 1);
  vec f0_x_mc = f0_c_mc/2;
  
  // Set treatment to 0 and 1, and add column of 1'Syx.
  mat matX_mc = join_rows(ones_D, Z_mc, C_mc);
  ...
  for (int k = 0; k < Ky_mc; k++) {
    ...
    for (int r = 0; r < max_Kx_Sy_temp; r++) {
      ...
      // Calculate F(v|z,u;omega*_r|k) for continuous posttreatment confounder
      vec Vbeta_temp = Vbeta_mc.subcube(0, r, k, size(pv, 1, 1));
      double Vsig_temp = sqrt(Vsig2_mc(0, r, k));
      V_mc(ind_Syx_mc) = randn(num_Syx_mc) * Vsig_temp + (matX_mc.rows(ind_Syx_mc) * Vbeta_temp);
    }
  }
  mat matV_mc = join_rows(ones_D, Z_mc, V_mc, C_mc);
  ...
  // Draw mediation for each of the D observations--------------------
  // Initialize
  mat lambda_M_mc = zeros(D, (Ky_mc + 1));
  ...
  for (int k = 0; k < Ky_mc; k++) {
    ...
    for (int r = 0; r < max_Kx_Sy_temp; r++) {
      ...
      for (int q = 0; q < p_Z; q++) {
        for (int d = 0; d < D; d++) {
          double dbinom_temp = R::dbinom(Z_mc(d), 1, Zpi_mc(q, r, k), false);
          prob_c_mc(d) = prob_c_mc(d) * dbinom_temp;
        }
      }
      // Calculate sum_{r=1}^{K_r} \frac{n_{rk}}{alpha_omega+n_k}*f(z,c;omega*_r|k)
      // (This is part of the denominator of E[ Y | M=m, Z=z, C=c, theta^star, omega^star, Syx])
      vec Vbeta_temp = Vbeta_mc.subcube(0, r, k, size(pv, 1, 1));
      vec Vsig_temp = sqrt(Vsig2_mc(0, r, k)) * ones_D;
      vec meanV_temp = matX_mc * Vbeta_temp;
      double prob_omega_rk_mc = (n_k_temp * n_rk_temp)/(alpha_omega + n_k_temp);
      sum_f_xv_mc += prob_omega_rk_mc * prob_c_mc % normpdf(V_mc, meanV_temp, Vsig_temp);
    }
    double prob_omega_ky_mc = (n_k_temp * alpha_omega)/(alpha_omega + n_k_temp);
    lambda_M_mc.col(k) = prob_omega_ky_mc * f0_x_mc + sum_f_xv_mc;
  }
  double prob_theta_Kyx_mc = alpha_theta;
  lambda_M_mc.col(Ky_mc) = prob_theta_Kyx_mc * f0_x_mc;
  ...
  for (int k = 0; k < Ky_mc; k++) {
    ...
    uvec ind_Sy_mc = find(Sy_mc == (k+1));
    int num_Sy_mc = ind_Sy_mc.n_elem;
    M_mc(ind_Sy_mc) = randn(num_Sy_mc) * Msig_temp + (matV_mc.rows(ind_Sy_mc) * Mbeta_temp);
  }
  
  mat M_temp = join_rows(Z_mc % M_mc, M_mc, V_mc, C_mc);
  mat matM_mc = join_rows(ones_D, Z_mc, M_temp);
  
  // End draw of mediation------------------------------------
  
  return List::create(_["Z_mc"] = Z_mc,
                      _["M_mc"] = M_mc);
