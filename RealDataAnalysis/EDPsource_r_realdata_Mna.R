...
EDPmediationMCMC = function(M, V, Z, C, 
  ...
  Ky = 2
  Sy = numeric(N)
  Sy[-ind_naV] = kmeans(cbind(M, V, Z, scaC)[-ind_naV,], Ky)$cluster
  Sy[ind_naV] = rbinom(n_naV,1,0.5) + 1
  Syx[,1] = Sy
  ...
  for (num in 1:num_MC_prior) {
    ...
    matXVbeta_temp = matX %*% Vbeta_temp
    V_temptemp = V
    # V_temptemp[ind_naV] = runif(n_naV,Vmin,Vmax)
    V_temptemp[ind_naV] = rnorm(n_naV,matXVbeta_temp,sqrt(Vsig2_temp))
    
    f0_v = f0_v + dnorm(V_temptemp, matXVbeta_temp, sqrt(Vsig2_temp))
  }
  f0_v = f0_v/num_MC_prior
  # f0_v[ind_naV] = 1
  ...
  for (num in 1:num_MC_prior) {
    ...
    # V_temptemp[ind_naV] = runif(n_naV,Vmin,Vmax)
    V_temptemp[ind_naV] = rnorm(n_naV,matXVbeta_temp,sqrt(Vsig2_temp))
    matV_temptemp = cbind(1, Z, V_temptemp, scaC)

    Msig2_temp = rscainvchisq_cpp(1, a_Msig2, b_Msig2)
    Mbeta_temp = rmvn_cpp(1, a_Mbeta, B_Mbeta)
    matVMbeta_temp = matV_temptemp %*% Mbeta_temp
  ...
  for (gibbs_reps in 1:gibbs_total) {
    ...
    cluster_res = EDPcluster_concon(N, p_Z, p_C1, p_C2, is_naV, 
                                    M, V, X, matV, matX,
                                    Sy, Sx, unique_Syx,
                                    MbetaPars, Msig2Pars,
                                    VbetaPars, Vsig2Pars,
    ...
    Vna_new = numeric(n_naV)
    for (i in 1:n_naV) {
      nas = ind_naV[i]
      ind_Sy_nas = which(unique_Sy == Sy[nas])
      ind_Syx_nas = which(apply((t(unique_Syx) == c(Sy[nas], Sx[nas])), 2, prod) == 1)

      # Vna_new[i] = rnorm(1, c(matX[nas,] %*% VbetaPars[,ind_Syx_nas]), sqrt(Vsig2Pars[ind_Syx_nas]))
      
      M1beta = MbetaPars[3,ind_Sy_nas]
      Msig2_temp = Msig2Pars[ind_Sy_nas]
      Vsig2_temp = Vsig2Pars[ind_Syx_nas]
      
      M0hat = c(matV[nas,-3] %*% MbetaPars[-3,ind_Sy_nas])
      V0hat = c(matX[nas,] %*% VbetaPars[,ind_Syx_nas])
      
      OMEGA[1,1] = 1/Msig2_temp
      OMEGA[2,2] = 1/Vsig2_temp + (M1beta)^{2}/Msig2_temp
      OMEGA[1,2] = (M0hat*M1beta/Msig2_temp)
      OMEGA[2,1] = OMEGA[1,2]
      
      MU[1] = M0hat
      MU[2] = c(V0hat/Vsig2_temp - M0hat*M1beta/Msig2_temp)/OMEGA[2,2]
      
      while (is_sympd(OMEGA)==FALSE) {
        OMEGA = OMEGA + 0.01*diag(2)
      }
      SIGMA = inv_cpp(OMEGA)
      RHO_MV = (SIGMA[1,2]/sqrt(SIGMA[1,1]*SIGMA[2,2]))
      
      # NA V and Y (inegrated out Y and conditional on M = m)
      meanV = MU[2] + (SIGMA[1,2]/SIGMA[1,1])*(M[nas] - MU[1])
      vcovV = SIGMA[2,2] - SIGMA[1,2]^{2} / SIGMA[1,1]
      
      Vna_new[i] = rnorm(1, meanV, sqrt(vcovV))
      
      # if (vcovV > 0) {
      #   Vna_new[i] = rnorm(1, meanV, sqrt(vcovV))
      # } else if (SIGMA[2,2] > 0) {
      #   Vna_new[i] = rnorm(1, V0hat, sqrt(SIGMA[2,2]))
      # } else {
      #   Vna_new[i] = rnorm(1, V0hat, sqrt(Vsig2Pars[ind_Syx_nas]))
      # }
    }
    ...
    {
      V_temptemp = V
      V_temptemp[ind_naV] = Vna_new
      
      count = 1
      for (k in unique_Sy) {
        ind_Sy = which(Sy == k)
        
        v_temp_k = V_temptemp[ind_Sy]
        matX_temp_k = matrix(matX[ind_Sy,],ncol=pv)
    ...
    for (k in unique_Sy) {
      ...
      Mregression_list = update_reg_con(M[ind_Sy],matV_temptemp[ind_Sy,],MbetaPars[,k],Msig2Pars[,k],
                                        a_Msig2,b_Msig2,Binv_Mbeta,aBinv_Mbeta)
      Msig2Pars[,k] = Mregression_list$sig2_par
      MbetaPars[,k] = Mregression_list$beta_par
    }
...
EDPmediationDISTm = function(MCMCresult, esttype, save_cluster){
  ...
  for (post_reps in 1:n_MCMC) {
    ...
    MCresult = EDPpostmc_M(p_C1, p_C2,
                                n_k, n_rk, Ky, Kyx, max_Kx_Sy, 
                                D, num_MC, rho_mc,
                                YbetaPars, Ysig2Pars,
                                MbetaPars, Msig2Pars,
    ...
    Z_mc[[post_reps]] = MCresult$Z_mc
    M_mc[[post_reps]] = MCresult$M_mc
  ...
  POSTresult = list(Z_mc = Z_mc, M_mc = M_mc)
