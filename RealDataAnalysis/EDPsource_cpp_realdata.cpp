...
List EDPcluster_concon(int const& N, int const& p_Z, int const& p_C1, int const& p_C2,
                       vec const& is_naY, vec const& is_naV, 
                       vec const& is_naVorY, vec const& is_naVandY, vec const& is_naVnotY, 
                       vec const& Y, vec const& M, vec const& V, mat const& X,
                       mat const& matM, mat const& matV, mat const& matX,
                       uvec Sy, uvec Sx, umat unique_Syx, 
                       mat YbetaPars, mat Ysig2Pars,
                       ...
                       vec const& f0_y, vec const& f0_m, vec const& f0_v, vec const& f0_x,
                       double const& a_Ysig2, double const& b_Ysig2,
                       vec const& a_Ybeta, mat const& B_Ybeta, 
                       mat const& Binv_Ybeta, mat const& aBinv_Ybeta, 
  ...
  double Ylikreg, Mlikreg, Vlikreg, Xlik;
  ...
  List outcome_list, mediator_list, posttrt_con_list, con_con_list;
  
  double Zpi_prop, Xpi_prop, Xmu_prop, Xtau2_prop;
  vec Ybeta_prop, Mbeta_prop, Vbeta_prop, Ysig_prop, Msig_prop, Vsig_prop;
  double Ysig2_prop, Msig2_prop, Vsig2_prop;
  vec Ybeta_curr, Mbeta_curr, Ysig_curr, Msig_curr;
  
  double Y_temp, M_temp, V_temp;
  rowvec matM_temp, matV_temp, matX_temp, X_temp;
  ...
  for (int i = 0; i < N; i++) {
    ...
    for (int k = 0; k < Ky; k++) {
      ...
      if (is_naVorY(i) == 0){
      ...
      } else if (is_naVandY(i) == 1){
        // NAs V and Y (integrated out V and Y)
        // Ylikreg <- Ylikreg * Mlikreg * Vlikreg (see Vlikreg)
        // Ylikreg = f0_y(i);
        // Mlikreg = f0_m(i);
        Ylikreg = 1;
        Mlikreg = 1;
      } else if (is_naVnotY(i) == 1){
        // NAs V, but not Y (integrated out V)
        // Ylikreg <- Ylikreg * Mlikreg * Vlikreg (see Vlikreg)
        // Ylikreg = f0_y(i);
        // Mlikreg = f0_m(i);
        Ylikreg = 1;
        ...
        // NAs Y, but not V (integrated out Y)
        // Ylikreg <- Ylikreg * Mlikreg (see Vlikreg)
        // Ylikreg = f0_y(i);
        Ylikreg = 1;
        Mlikreg = normpdf(M_temp,dot(matV_temp,MbetaPars.col(k)),sqrt(Msig2Pars(k)));
        ...
        // mat SIGMA(2,2);
        // 
        // double Y1beta = X_temp(0)*YbetaPars(2,k) + YbetaPars(3,k);
        // mat YbetaPars_temp = YbetaPars; YbetaPars_temp.shed_row(2); YbetaPars_temp.shed_row(3);
        // double Y0hat = dot(matV_temp, YbetaPars_temp.col(k));
        // double M0hat = dot(matV_temp, MbetaPars.col(k));
        // 
        // SIGMA(0,0) = Ysig2Pars(k) + Msig2Pars(k) * pow(Y1beta,2)
        // SIGMA(1,1) = Msig2Pars(k);
        // SIGMA(0,1) = Msig2Pars(k) * Y1beta;
        // SIGMA(1,0) = SIGMA(0,1);
        // SIGMA %= {{ 1, -1}, { -1, 1}};
        // 
        // MU(1) = M0hat;
        // MU(0) = Y0hat + MU(1) * Y1beta;
        // 
        // Mlikreg = normpdf(M_temp,MU(1),sqrt(SIGMA(1,1)));
      }
      ...
      for (int r = 0; r < Kx_Sy; r++) {
        ...
        if (is_naVorY(i) == 0){
          ...
          double SIGMA_M = Msig2Pars(k) + Vsig2Pars(count) * pow(M1beta,2);
          double MU_M = dot(matX_temp, MbetaPars_temp.col(k)) + dot(matX_temp, VbetaPars.col(count)) * M1beta;
          
          Mlikreg = normpdf(M_temp,MU_M,sqrt(SIGMA_M));
          ...
          // 
          // double Y1beta = X_temp(0)*YbetaPars(2,k) + YbetaPars(3,k);
          // double Y2beta = YbetaPars(4,k);
          // double M1beta = MbetaPars(2,k);
          // 
          // mat MbetaPars_temp = MbetaPars; MbetaPars_temp.shed_row(2);
          // mat YbetaPars_temp = YbetaPars; YbetaPars_temp.shed_row(2); YbetaPars_temp.shed_row(3); YbetaPars_temp.shed_row(4);
          // double Y0hat = dot(matX_temp, YbetaPars_temp.col(k));
          // double M0hat = dot(matX_temp, MbetaPars_temp.col(k));
          // double V0hat = dot(matX_temp, VbetaPars.col(count));
          // 
          // // OMEGA(0,0) = 1/Ysig2Pars(k);
          // // OMEGA(1,1) = 1/Msig2Pars(k) + pow(Y1beta,2)/Ysig2Pars(k);
          // // OMEGA(2,2) = 1/Vsig2Pars(count) + pow(M1beta,2)/Msig2Pars(k) + pow(Y2beta,2)/Ysig2Pars(k);
          // // OMEGA(0,1) = (- Y1beta/Ysig2Pars(k));
          // // OMEGA(0,2) = (- Y2beta/Ysig2Pars(k));
          // // OMEGA(1,2) = (Y1beta*Y2beta/Ysig2Pars(k) - M1beta/Msig2Pars(k));
          // // OMEGA(1,0) = OMEGA(0,1);
          // // OMEGA(2,0) = OMEGA(0,2);
          // // OMEGA(2,1) = OMEGA(1,2);
          // 
          // SIGMA(0,0) = Ysig2Pars(k) + Msig2Pars(k) * pow(Y1beta,2) + Vsig2Pars(count) * pow(Y2beta + Y1beta * M1beta,2);
          // SIGMA(1,1) = Msig2Pars(k) + Vsig2Pars(count) * pow(M1beta,2);
          // SIGMA(2,2) = Vsig2Pars(count);
          // SIGMA(0,1) = - Msig2Pars(k) * Y1beta - Vsig2Pars(count) * (Y1beta * pow(M1beta,2) + Y2beta * M1beta);
          // SIGMA(0,2) = Vsig2Pars(count) * (Y1beta * M1beta + Y2beta);
          // SIGMA(1,2) = - Vsig2Pars(count) * M1beta;
          // SIGMA(1,0) = SIGMA(0,1);
          // SIGMA(2,0) = SIGMA(0,2);
          // SIGMA(2,1) = SIGMA(1,2);
          ...
          // 
          // MU(2) = V0hat;
          // MU(1) = M0hat + MU(2) * M1beta;
          // MU(0) = Y0hat + MU(1) * Y1beta + MU(2) *Y2beta;
          // 
          // Mlikreg = normpdf(M_temp,MU(1),sqrt(SIGMA(1,1)));
        } else if (is_naVnotY(i) == 1){
          // NAs V, but not Y (integrated out V)
          ...
          MU(1) = M0hat + V0hat * M1beta;
          MU(0) = Y0hat + MU(1) * Y1beta + V0hat * Y2beta;
          
          // double rho2_YM = pow(SIGMA(0,1),2)/(SIGMA(0,0)*SIGMA(1,1));
          // Mlikreg = normpdf(M_temp, MU(1), sqrt(SIGMA(1,1)));
          // Ylikreg = normpdf(Y_temp, (MU(0) + (SIGMA(0,1)/SIGMA(1,1))*(M_temp - MU(1))), sqrt((1-rho2_YM)*SIGMA(0,0)));
          
          Mlikreg = 1;
          Ylikreg = dmvn_cpp({Y_temp, M_temp}, MU, SIGMA, false);
        ...
        probs(count) = (n_k_woi * (n_rk_woi/(n_k_woi+alpha_omega))) * Ylikreg * Mlikreg * Vlikreg * Xlik;
      ...
      // ---------------------------------------------------------------------------------------
      probs(Kyx+k) = (n_k_woi * (alpha_omega/(n_k_woi+alpha_omega))) * Ylikreg * Mlikreg * f0_v(i) * f0_x(i);
    }
    
    // ---------------------------------------------------------------------------------------
    // ---------------------------------- New y&m-x clusters ---------------------------------
    // ---------------------------------------------------------------------------------------
    probs(Kyx+Ky) = alpha_theta * f0_y(i) * f0_m(i) * f0_v(i) * f0_x(i);
  ...
  if (Kyx>Ky && Ky>1){
    ...
    // ind_Syx = find(Sy == k_curr && Sx == r_curr && is_naY == 0);
    // ind_Syx = find(Sy == k_curr && Sx == r_curr && is_naVorY == 0);
    num_Syx = ind_Syx.size();
    
    vec Y_temp = Y(ind_Syx);
    vec M_temp = M(ind_Syx);
    mat matM_temp = matM.rows(ind_Syx);
    mat matV_temp = matV.rows(ind_Syx);
    ...
    ratio =
      ...
      as_scalar((sum(log_normpdf(Y_temp, matM_temp * Ybeta_prop, Ysig_prop)) + sum(log_normpdf(M_temp, matV_temp * Mbeta_prop, Msig_prop))) -
      (sum(log_normpdf(Y_temp, matM_temp * Ybeta_curr, Ysig_curr)) + sum(log_normpdf(M_temp, matV_temp * Mbeta_curr, Msig_curr)))) +
  ...
  if (randu(1)[0] < 0.5){
    ...
    if (Kyx>Ky){
      ...
      ratio =
        ...
        as_scalar((sum(log(f0_y(ind_Syx) % f0_m(ind_Syx)))) - (sum(log_normpdf(Y_temp, matM_temp * Ybeta_curr, Ysig_curr)) + sum(log_normpdf(M_temp, matV_temp * Mbeta_curr, Msig_curr)))) +
    ...
    if (num_max_Kx_Sy>0 && Ky>1){
      ...
      ratio =
        ...
        as_scalar((sum(log_normpdf(Y_temp, matM_temp * Ybeta_prop, Ysig_prop)) + sum(log_normpdf(M_temp, matV_temp * Mbeta_prop, Msig_prop))) - (sum(log(f0_y(ind_Syx) % f0_m(ind_Syx))))) -
...
List EDPpostmc_concon(int const& p_C1, int const& p_C2,
  ...
  double Ysig2_prop, Msig2_prop, Vsig2_prop;
  vec Ybeta_prop, Mbeta_prop, Vbeta_prop;
  ...
  if (Km_mc > Km_poss) {
    ...
    Ysig2_prop = rscainvchisq_cpp(1, a_Ysig2, b_Ysig2)[0];
    Ybeta_prop = mvnrnd(a_Ybeta, B_Ybeta);
    Ybeta_mc.insert_cols(Km_poss, Ybeta_prop);
    Ysig2_mc.insert_cols(Km_poss, 1);
    Ysig2_mc(Km_poss) = Ysig2_prop;
  }
  ...
  // mat M0x0v0_temp = join_rows(z0_D, M0_mc, V0C_temp);
  // mat Mrx1v1_temp = join_rows(z1_D, Mr_mc, V1C_temp);
  // mat M1x1v1_temp = join_rows(z1_D, M1_mc, V1C_temp);
  mat M0x0v0_temp = join_rows(z0_D, z0_D % M0_mc, M0_mc, V0C_temp);
  mat Mrx1v1_temp = join_rows(z1_D, z1_D % Mr_mc, Mr_mc, V1C_temp);
  mat M1x1v1_temp = join_rows(z1_D, z1_D % M1_mc, M1_mc, V1C_temp);
  mat matM0_x0v0_mc = join_rows(ones_D, M0x0v0_temp);
  mat matMr_x1v1_mc = join_rows(ones_D, Mrx1v1_temp);
  mat matM1_x1v1_mc = join_rows(ones_D, M1x1v1_temp);
  ...
  // ---------------------------------------------------------------------------------------
  // ---------------------------------------------------------------------------------------
  // Find f0( M | Z, C, theta) integrating over G_0^theta.
  // Find E0[Y|M,Z,C,theta] integrating over G_0^theta.
  // Mbeta_prior and Msig2_prior was calculated before Gibbs sampling loop
  // Ybeta_prior and Ysig2_prior was calculated before Gibbs sampling loop
  // Compute f(M | Z=z, C=c, theta, omega, Syx) using D new draws
  // Compute E[Y|M=m,Z=z,C=c,theta,omega,Syx] using D new draws
  // Initialize
  vec f0_M0_x0v0_mc = zeros_D;
  vec f0_Mr_x1v1_mc = zeros_D;
  vec f0_M1_x1v1_mc = zeros_D;
  vec E0_Y_x0v0m0_mc = zeros_D;
  vec E0_Y_x1v1mr_mc = zeros_D;
  vec E0_Y_x1v1m1_mc = zeros_D;
  
  for (int num = 0; num < num_MC; num++) {
    vec Mbeta_temp = Mbeta_prior.col(num);
    vec Msig_temp = sqrt(Msig2_prior(num)) * ones_D;
    vec meanM_x0v0_temp = matV0_x0_mc * Mbeta_temp;
    vec meanM_x1v1_temp = matV1_x1_mc * Mbeta_temp;
    
    f0_M0_x0v0_mc += normpdf(M0_mc, meanM_x0v0_temp, Msig_temp);
    f0_Mr_x1v1_mc += normpdf(Mr_mc, meanM_x1v1_temp, Msig_temp);
    f0_M1_x1v1_mc += normpdf(M1_mc, meanM_x1v1_temp, Msig_temp);
    
    vec Ybeta_temp = Ybeta_prior.col(num);
    E0_Y_x0v0m0_mc += (matM0_x0v0_mc * Ybeta_temp);
    E0_Y_x1v1mr_mc += (matMr_x1v1_mc * Ybeta_temp);
    E0_Y_x1v1m1_mc += (matM1_x1v1_mc * Ybeta_temp);
  }
  f0_M0_x0v0_mc /= num_MC;
  f0_Mr_x1v1_mc /= num_MC;
  f0_M1_x1v1_mc /= num_MC;
  
  E0_Y_x0v0m0_mc /= num_MC;
  E0_Y_x1v1mr_mc /= num_MC;
  E0_Y_x1v1m1_mc /= num_MC;
  
  // Calculate E[Y_{z,M_z'}] using MC integration to integrate
  // E[Y | M=m, Z=z, C=c, theta^star, omega^star, Syx] over covariate distribution
  // Initialize
  vec numer_Y_x0v0m0_mc = zeros_D;
  vec denom_Y_x0v0m0_mc = zeros_D;
  vec numer_Y_x1v1mr_mc = zeros_D;
  vec denom_Y_x1v1mr_mc = zeros_D;
  vec numer_Y_x1v1m1_mc = zeros_D;
  vec denom_Y_x1v1m1_mc = zeros_D;
  ...
  for (int k = 0; k < Km_mc; k++) {
    ...
    vec meanY_x0v0m0_temp = matM0_x0v0_mc * Ybeta_temp;
    vec meanY_x1v1mr_temp = matMr_x1v1_mc * Ybeta_temp;
    vec meanY_x1v1m1_temp = matM1_x1v1_mc * Ybeta_temp;
    
    // Calculate lambda_k^y (m,z,c)
    vec lambda_Y_x0v0m0_ky_mc = lambda_M_x0v0_mc.col(k) % normpdf(M0_mc, meanM_x0v0_temp, Msig_temp);
    vec lambda_Y_x1v1mr_ky_mc = lambda_M_x1v1_mc.col(k) % normpdf(Mr_mc, meanM_x1v1_temp, Msig_temp);
    vec lambda_Y_x1v1m1_ky_mc = lambda_M_x1v1_mc.col(k) % normpdf(M1_mc, meanM_x1v1_temp, Msig_temp);
    
    // Calculate sum_(k=1)^{Ky} lambda_k^y (m,z,c)
    // (This is part of the numerator of E[Y | M=m, Z=z, C=c, theta^star, omega^star, Syx])
    numer_Y_x0v0m0_mc += lambda_Y_x0v0m0_ky_mc % meanY_x0v0m0_temp;
    numer_Y_x1v1mr_mc += lambda_Y_x1v1mr_ky_mc % meanY_x1v1mr_temp;
    numer_Y_x1v1m1_mc += lambda_Y_x1v1m1_ky_mc % meanY_x1v1m1_temp;
    
    // Calculate sum_(k=1)^{Ky} lambda_k^y (m,z,c)
    // (This is part of the denominator of E[Y | M=m, Z=z, C=c, theta^star, omega^star, Syx])
    denom_Y_x0v0m0_mc += lambda_Y_x0v0m0_ky_mc;
    denom_Y_x1v1mr_mc += lambda_Y_x1v1mr_ky_mc;
    denom_Y_x1v1m1_mc += lambda_Y_x1v1m1_ky_mc;
  }
  // Calculate lambda_{Ky+1}^{y} (m,z,c)
  vec lambda_Y_x0v0m0_Ky_mc = lambda_M_x0v0_Ky_mc % f0_M0_x0v0_mc;
  vec lambda_Y_x1v1mr_Ky_mc = lambda_M_x1v1_Ky_mc % f0_Mr_x1v1_mc;
  vec lambda_Y_x1v1m1_Ky_mc = lambda_M_x1v1_Ky_mc % f0_M1_x1v1_mc;
  
  // Numerator of E[Y | M=m, Z=z, C=c, theta^star, omega^star, Syx]
  numer_Y_x0v0m0_mc += lambda_Y_x0v0m0_Ky_mc % E0_Y_x0v0m0_mc;
  numer_Y_x1v1mr_mc += lambda_Y_x1v1mr_Ky_mc % E0_Y_x1v1mr_mc;
  numer_Y_x1v1m1_mc += lambda_Y_x1v1m1_Ky_mc % E0_Y_x1v1m1_mc;
  
  // Denominator of E[Y | M=m, Z=z, C=c, theta^star, omega^star, Syx]
  denom_Y_x0v0m0_mc += lambda_Y_x0v0m0_Ky_mc;
  denom_Y_x1v1mr_mc += lambda_Y_x1v1mr_Ky_mc;
  denom_Y_x1v1m1_mc += lambda_Y_x1v1m1_Ky_mc;
  ...
  // ---------------------------------------------------------------------------------------
  // Check 0 denominator
  uvec ind_Y_x0v0m0_mc = find(denom_Y_x0v0m0_mc == 0);
  uvec ind_Y_x1v1mr_mc = find(denom_Y_x1v1mr_mc == 0);
  uvec ind_Y_x1v1m1_mc = find(denom_Y_x1v1m1_mc == 0);
  denom_Y_x0v0m0_mc(ind_Y_x0v0m0_mc) = ones(ind_Y_x0v0m0_mc.n_elem);
  denom_Y_x1v1mr_mc(ind_Y_x1v1mr_mc) = ones(ind_Y_x1v1mr_mc.n_elem);
  denom_Y_x1v1m1_mc(ind_Y_x1v1m1_mc) = ones(ind_Y_x1v1m1_mc.n_elem);
  
  // Calculate E[Y_{0,M0}^{(d)}]
  vec E_Y_x0v0m0_mc = numer_Y_x0v0m0_mc/denom_Y_x0v0m0_mc;
  // Calculate E[Y_{1,Mr}^{(d)}]
  vec E_Y_x1v1mr_mc = numer_Y_x1v1mr_mc/denom_Y_x1v1mr_mc;
  // Calculate E[Y_{1,M1}^{(d)}]
  vec E_Y_x1v1m1_mc = numer_Y_x1v1m1_mc/denom_Y_x1v1m1_mc;
  
  // Check NaN
  uvec ind_E_Y_x0v0m0_mc = find_nan(E_Y_x0v0m0_mc);
  uvec ind_E_Y_x1v1mr_mc = find_nan(E_Y_x1v1mr_mc);
  uvec ind_E_Y_x1v1m1_mc = find_nan(E_Y_x1v1m1_mc);
  E_Y_x0v0m0_mc.shed_rows(ind_E_Y_x0v0m0_mc);
  E_Y_x1v1mr_mc.shed_rows(ind_E_Y_x1v1mr_mc);
  E_Y_x1v1m1_mc.shed_rows(ind_E_Y_x1v1m1_mc);
  
  // Calculate E[Y_{0,M0}]
  double E_Y_z0m0 = mean(E_Y_x0v0m0_mc);
  // Calculate E[Y_{1,Mr}]
  double E_Y_z1mr = mean(E_Y_x1v1mr_mc);
  // Calculate E[Y_{1,M1}]
  double E_Y_z1m1 = mean(E_Y_x1v1m1_mc);
  
  // Calculate E[Y_{1,M1}] - E[Y_{1,Mr}]
  double NIE = E_Y_z1m1 - E_Y_z1mr;
  // Calculate E[Y_{1,Mr}] - E[Y_{0,M0}]
  double NDE = E_Y_z1mr - E_Y_z0m0;
...
// [[Rcpp::export]]
List EDPpostmc_concon_condC(vec const& condC, int const& q_condC, int const& p_C1, int const& p_C2,
                            vec const& n_k, mat const& n_rk, 
                            int const& Ky, int const& Kyx, vec const& max_Kx_Sy, 
                            int const& D, int const& num_MC, vec const& rho_mc,
                            mat const& YbetaPars, mat const& Ysig2Pars,
  ...
  // ---------------------------------------------------------------------------------------
  // ---------------------------------------------------------------------------------------
  // Calculate P(c1|c2;theta_k^*,omega_r|k^*) for confounder c1 conditional on c2
  // Draw mediation for each of the D observations--------------------
  // Initialize
  vec f0_c2_mc = 0.5 * ones_D;
  mat lambda_C1_c2_mc = zeros(D, (Kyx + Ky + 1));
  ...
  for (int k = 0; k < Ky; k++) {
    ...
    for (int r = 0; r < max_Kx_Sy_temp; r++) {
      int n_rk_temp = n_rk(k,r);
      
      vec prob_c_mc = ones_D;
      // Calculate f(z,c;omega*_r|k) for binary confounders
      for (int d = 0; d < D; d++) {
        double dbinom_temp = R::dbinom(condC(d), 1, Xpi_mc(q_condC-1, r, k), false);
        prob_c_mc(d) = prob_c_mc(d) * dbinom_temp;
      }
      // // Calculate f(z,c;omega*_r|k) for continuous confounders
      // for (int q = 0; q < p_C2; q++) {
      ...
      // (This is part of the denominator of E[ Y | M=m, Z=z, C=c, theta^star, omega^star, Syx])
      double prob_omega_rk_mc = (n_k_temp * n_rk_temp)/(alpha_omega + n_k_temp);
      lambda_C1_c2_mc.col(count) = prob_omega_rk_mc * prob_c_mc;
      
      count++;
    }
    double prob_omega_ky_mc = (n_k_temp * alpha_omega)/(alpha_omega + n_k_temp);
    lambda_C1_c2_mc.col(count) = prob_omega_ky_mc * f0_c2_mc;
    
    count++;
  }
  double prob_theta_Kyx_mc = alpha_theta;
  lambda_C1_c2_mc.col(Kyx + Ky) = prob_theta_Kyx_mc * f0_c2_mc;
  
  uvec Sx_cond_temp = uones_D;
  for (int d = 0; d < D; d++) {
    vec prob_C1_c2_temp = lambda_C1_c2_mc.row(d).t();
    Sx_cond_temp(d) = rmultinom_cpp(prob_C1_c2_temp);
  }
  
  uvec Sy_cond_mc = uones_D;
  uvec Sx_cond_mc = uones_D;
  for (int d = 0; d < D; d++) {
    count = 1;
    for (int k = 0; k < Ky; k++) {
      int max_Kx_Sy_temp = max_Kx_Sy(k) + 1;
      for (int r = 0; r < max_Kx_Sy_temp; r++) {
        if (count == Sx_cond_temp(d)){
          Sy_cond_mc(d) = k + 1;
          Sx_cond_mc(d) = r + 1;
        }
        count++;
  ...
  for (int k = 0; k < Ky_mc; k++) {
    ...
    for (int r = 0; r < max_Kx_Sy_temp; r++) {
      uvec ind_Syx_mc = find(Sy_cond_mc == (k+1) && Sx_cond_mc == (r+1));
      int num_Syx_mc = ind_Syx_mc.n_elem;
      n_rk_mc(k,r) = num_Syx_mc;
  ...
  // lambda_M_x0v0_mc.shed_col(Km_poss);
  // lambda_M_x0vr_mc.shed_col(Km_poss);
  // lambda_M_x1v1_mc.shed_col(Km_poss);
