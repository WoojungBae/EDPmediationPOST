...
# Define function to update outcome reg_coefficients in regression.
update_reg_con = function(Y_temp,matX_temp,beta_curr,sig2_curr,
                           a_sig2,b_sig2,Binv_beta,aBinv_beta) {
  n_temp = length(Y_temp)
  p_temp = length(beta_curr)
  matX_temp = matrix(matX_temp, ncol = p_temp)
  
  ind_naY_temp = which(apply(is.na(cbind(Y_temp,matX_temp)),1,sum)>0)
  n_naY_temp = length(ind_naY_temp)
  
  if (n_naY_temp == n_temp) {
    return(list(beta_par = beta_curr, sig2_par = sig2_curr))
  } else if (n_naY_temp > 0) {
    Y_temp = Y_temp[-ind_naY_temp]
    matX_temp = matX_temp[-ind_naY_temp,]
    matX_temp = matrix(matX_temp, ncol = p_temp)
  }
...
impute_V = function(Y_temp, M_temp, V_temp, Z_temp, matM_temp, matV_temp, matX_temp, 
                    n_naV_temp, ind_naV_temp, ind_naVandY_temp, unique_Sy_temp, unique_Syx_temp, Sy_temp, Sx_temp, 
                    YbetaPars_temp, Ysig2Pars_temp, MbetaPars_temp, Msig2Pars_temp, VbetaPars_temp, Vsig2Pars_temp,
                    minimum = -Inf, maximum = Inf){
  
  na_new = rep(NA,n_naV_temp)
  
  MU = numeric(3)
  SIGMA = matrix(NA,nrow=3,ncol=3)
  # OMEGA = matrix(NA,nrow=3,ncol=3)
  for (i in 1:n_naV_temp) {
    NAs = ind_naV_temp[i]
    ind_Sy_NAs = which(unique_Sy_temp == Sy_temp[NAs])
    ind_Syx_NAs = which(apply((t(unique_Syx_temp) == c(Sy_temp[NAs], Sx_temp[NAs])), 2, prod) == 1)
    
    na_new[i] = rnorm(1, c(matX_temp[NAs,] %*% VbetaPars_temp[,ind_Syx_NAs]), sqrt(Vsig2Pars_temp[ind_Syx_NAs]))
    
    Y1beta = Z_temp[NAs]*YbetaPars_temp[3,ind_Sy_NAs] + YbetaPars_temp[4,ind_Sy_NAs]
    Y2beta = YbetaPars_temp[5,ind_Sy_NAs]
    M1beta = MbetaPars_temp[3,ind_Sy_NAs]
    
    Y0hat = c(matM_temp[NAs,-(3:5)] %*% YbetaPars_temp[-(3:5),ind_Sy_NAs])
    M0hat = c(matV_temp[NAs,-3] %*% MbetaPars_temp[-3,ind_Sy_NAs])
    V0hat = c(matX_temp[NAs,] %*% VbetaPars_temp[,ind_Syx_NAs])
    
    # OMEGA[1,1] = 1/Ysig2Pars_temp[ind_Sy_NAs]
    # OMEGA[2,2] = 1/Msig2Pars_temp[ind_Sy_NAs] + (Y1beta)^{2}/Ysig2Pars_temp[ind_Sy_NAs]
    # OMEGA[3,3] = 1/Vsig2Pars_temp[ind_Syx_NAs] + (M1beta)^{2}/Msig2Pars_temp[ind_Sy_NAs] + (Y2beta)^{2}/Ysig2Pars_temp[ind_Sy_NAs]
    # OMEGA[1,2] = (- Y1beta/Ysig2Pars_temp[ind_Sy_NAs])
    # OMEGA[1,3] = (- Y2beta/Ysig2Pars_temp[ind_Sy_NAs])
    # OMEGA[2,3] = (Y1beta*Y2beta/Ysig2Pars_temp[ind_Sy_NAs] - M1beta/Msig2Pars_temp[ind_Sy_NAs])
    # OMEGA[2,1] = OMEGA[1,2]
    # OMEGA[3,1] = OMEGA[1,3]
    # OMEGA[3,2] = OMEGA[2,3]
    # # SIGMA = inv_cpp(OMEGA)
    
    SIGMA[1,1] = Ysig2Pars_temp[ind_Sy_NAs] + Msig2Pars_temp[ind_Sy_NAs] * Y1beta^{2} + Vsig2Pars_temp[ind_Syx_NAs] * (Y2beta + Y1beta * M1beta)^{2}
    SIGMA[2,2] = Msig2Pars_temp[ind_Sy_NAs] + Vsig2Pars_temp[ind_Syx_NAs] * M1beta^{2}
    SIGMA[3,3] = Vsig2Pars_temp[ind_Syx_NAs]
    SIGMA[1,2] = - Msig2Pars_temp[ind_Sy_NAs] * Y1beta - Vsig2Pars_temp[ind_Syx_NAs] * (Y1beta * M1beta^{2} + Y2beta * M1beta)
    SIGMA[1,3] = Vsig2Pars_temp[ind_Syx_NAs] * (Y1beta * M1beta + Y2beta)
    SIGMA[2,3] = - Vsig2Pars_temp[ind_Syx_NAs] * M1beta
    SIGMA[2,1] = SIGMA[1,2]
    SIGMA[3,1] = SIGMA[1,3]
    SIGMA[3,2] = SIGMA[2,3]
    SIGMA = SIGMA * matrix(c(c(1,-1,1),c(-1,1,-1),c(1,-1,1)),nrow=3)
    
    MU[3] = V0hat
    MU[2] = M0hat + MU[3] * M1beta
    MU[1] = Y0hat + MU[2] * Y1beta + MU[3] * Y2beta
    
    if (sum(ind_naVandY_temp == NAs) > 0){
      # NA V_temp and Y_temp (inegrated out Y_temp and conditional on M_temp = m)
      meanV = MU[3] + (SIGMA[2,3]/SIGMA[2,2])*(M_temp[NAs] - MU[2])
      vcovV = SIGMA[3,3] - (SIGMA[2,3]^{2})/SIGMA[2,2]
    } else {
      # NA V_temp, but not Y_temp (conditional on M_temp = m and Y_temp = y)
      temp = SIGMA[3,1:2] %*% inv_cpp(SIGMA[1:2,1:2])
      meanV = MU[3] + c(temp %*% (c(Y_temp[NAs], M_temp[NAs]) - MU[1:2]))
      vcovV = SIGMA[3,3] - c(temp %*% SIGMA[1:2,3])
    }
    
    if (vcovV>0) {
      na_temp = rnorm(1, meanV, sqrt(vcovV))
    } else {
      na_temp = rnorm(1, meanV, sqrt(SIGMA[3,3]))
    }
    
    na_new[i] = na_temp
    
    # if (na_temp < minimum) {
    #   na_new[i] = minimum - runif(1)
    # } else if (na_temp > maximum) {
    #   na_new[i] = maximum + runif(1)
    # } 
  }
...
impute_Y = function(Y_temp, M_temp, V_temp, Z_temp, matM_temp, matV_temp, matX_temp, 
                    n_naY_temp, ind_naY_temp, ind_naVandY_temp, unique_Sy_temp, unique_Syx_temp, Sy_temp, Sx_temp, 
                    YbetaPars_temp, Ysig2Pars_temp, MbetaPars_temp, Msig2Pars_temp, VbetaPars_temp, Vsig2Pars_temp,
                    minimum = -Inf, maximum = Inf){
  
  na_new = numeric(n_naY_temp)
  
  MU = numeric(3)
  SIGMA = matrix(NA,nrow=3,ncol=3)
  # OMEGA = matrix(NA,nrow=3,ncol=3)
  for (i in 1:n_naY_temp) {
    NAs = ind_naY_temp[i]
    ind_Sy_NAs = which(unique_Sy_temp == Sy_temp[NAs])
    ind_Syx_NAs = which(apply((t(unique_Syx_temp) == c(Sy_temp[NAs], Sx_temp[NAs])), 2, prod) == 1)
    ...
    # OMEGA[1,1] = 1/Ysig2Pars[ind_Sy_NAs]
    # OMEGA[2,2] = 1/Msig2Pars[ind_Sy_NAs] + (Y1beta)^{2}/Ysig2Pars[ind_Sy_NAs]
    # OMEGA[3,3] = 1/Vsig2Pars[ind_Syx_NAs] + (M1beta)^{2}/Msig2Pars[ind_Sy_NAs] + (Y2beta)^{2}/Ysig2Pars[ind_Sy_NAs]
    # OMEGA[1,2] = (- Y1beta/Ysig2Pars[ind_Sy_NAs])
    # OMEGA[1,3] = (- Y2beta/Ysig2Pars[ind_Sy_NAs])
    # OMEGA[2,3] = (Y1beta*Y2beta/Ysig2Pars[ind_Sy_NAs] - M1beta/Msig2Pars[ind_Sy_NAs])
    # OMEGA[2,1] = OMEGA[1,2]
    # OMEGA[3,1] = OMEGA[1,3]
    # OMEGA[3,2] = OMEGA[2,3]
    # # SIGMA = inv_cpp(OMEGA)
    ...
    if (sum(ind_naVandY_temp == NAs) > 0){
      # NA V and Y (inegrated out Y and conditional on M = m)
      meanY = MU[1] + SIGMA[1,2]/SIGMA[2,2]*(M[NAs] - MU[2])
      vcovY = SIGMA[1,1] - (SIGMA[1,2]^{2})/SIGMA[2,2]
    } else {
      # NA V, but not Y (conditional on M = m and Y = y)
      temp = SIGMA[1,2:3] %*% inv_cpp(SIGMA[2:3,2:3])
      meanY = MU[1] + c(temp %*% (c(M[NAs], V[NAs]) - MU[2:3]))
      vcovY = SIGMA[1,1] - c(temp %*% SIGMA[2:3,1])
    }
    
    if (vcovY>0) {
      na_temp = rnorm(1, meanY, sqrt(vcovY))
    } else {
      na_temp = rnorm(1, meanY, sqrt(SIGMA[1,1]))
    }
    
    na_new[i] = na_temp
    
    # if (na_temp < Ymin) {
    #   na_new[i] = Ymin - runif(1)
    # } else if (na_temp > Ymax) {
    #   na_new[i] = Ymax + runif(1)
    # }
  }
...
generate_data = function(N,Scenario){
  ...
  if (cond123||cond456){
    ...
    if (Scenario==1||Scenario==4){
      ...
      omega_M=3
      alpha_M=10
      xi_M0=1.0+1.7*z0+0.05*V0+0.4*C1+0.9*C2
      xi_M1=1.0+1.7*z1+0.05*V1+0.4*C1+0.9*C2
      ...
      omega_M=3
      alpha_M=10
      xi_M0=1.0+1.7*z0+0.05*V0+0.4*C1+0.3*C2
      xi_M1=1.0+1.7*z1+0.05*V1+0.4*C1+0.3*C2
      M0=rsn(N,xi=xi_M0,omega=omega_M,alpha=alpha_M)
      M1=rsn(N,xi=xi_M1,omega=omega_M,alpha=alpha_M)
      ...
      omega_M=1
      alpha_M=7
      xi_M0=-0.3+0.2*z0+0.05*V0+0.1*C1+0.3*C2
      xi_M1=-0.3+0.2*z1+0.05*V1+0.1*C1+0.3*C2
      M0=rsn(N,xi=xi_M0,omega=omega_M,alpha=alpha_M)
      M1=rsn(N,xi=xi_M1,omega=omega_M,alpha=alpha_M)
      ...
      mu_Y_z0m0=5+2.5*z0+0.2*step(M0,1.7)+1.0*M0^{2}+0.3*V0+0.4*C1+0.3*C2
      mu_Y_z1m0=5+2.5*z1+0.2*step(M0,1.7)+1.0*M0^{2}+0.3*V1+0.4*C1+0.3*C2
      mu_Y_z1m1=5+2.5*z1+0.2*step(M1,1.7)+1.0*M1^{2}+0.3*V1+0.4*C1+0.3*C2
    ...
    if (Scenario==7||Scenario==10){
      ...
      omega_M=3
      alpha_M=10
      xi_M0=-1.0+1.7*z0+0.05*V0+0.3*C[,1]+0.1*C[,4]-0.2*C[,7]-0.4*C[,10]+0.6*C[,13]
      xi_M1=-1.0+1.7*z1+0.05*V1+0.3*C[,1]+0.1*C[,4]-0.2*C[,7]-0.4*C[,10]+0.6*C[,13]
      M0=rsn(N,xi=xi_M0,omega=omega_M,alpha=alpha_M)
      M1=rsn(N,xi=xi_M1,omega=omega_M,alpha=alpha_M)
      ...
      omega_M=1
      alpha_M=7
      xi_M0=-0.8+0.2*z0+0.05*V0+0.5*C[,1]+0.6*C[,4]-0.2*C[,7]-0.4*C[,10]+0.6*C[,13]
      xi_M1=-0.8+0.2*z1+0.05*V1+0.5*C[,1]+0.6*C[,4]-0.2*C[,7]-0.4*C[,10]+0.6*C[,13]
      M0=rsn(N,xi=xi_M0,omega=omega_M,alpha=alpha_M)
      M1=rsn(N,xi=xi_M1,omega=omega_M,alpha=alpha_M)
      ...
      mu_Y_z0m0=5+2.5*z0+0.2*step(M0,1.7)+1.0*M0^{2}+0.3*V0+0.2*C[,2]+0.3*C[,5]-0.4*C[,8]-0.2*C[,11]+0.6*C[,14]
      mu_Y_z1m0=5+2.5*z1+0.2*step(M0,1.7)+1.0*M0^{2}+0.3*V1+0.2*C[,2]+0.3*C[,5]-0.4*C[,8]-0.2*C[,11]+0.6*C[,14]
      mu_Y_z1m1=5+2.5*z1+0.2*step(M1,1.7)+1.0*M1^{2}+0.3*V1+0.2*C[,2]+0.3*C[,5]-0.4*C[,8]-0.2*C[,11]+0.6*C[,14]
...
EDPmediationMCMC = function(Y, M, V, Z, C, 
  ...
  # NA: Y
  is_naY = is.na(Y)*1
  ind_naY = which(is_naY==1)
  n_naY = length(ind_naY)
  
  # NA: V
  is_naV = is.na(V)*1
  ind_naV = which(is_naV==1)
  n_naV = length(ind_naV)
  
  # NA: V and Y
  is_naVandY = is_naV * is_naY
  ind_naVandY = which(is_naVandY==1)
  n_naVandY = length(ind_naVandY)
  
  # NA: V or Y
  is_naVorY = is.na(V+Y)*1
  ind_naVorY = which(is_naVorY==1)
  n_naVorY = length(ind_naVorY)
  
  # NA: V, but not Y
  is_naVnotY = is_naV * (1-is_naY)
  ind_naVnotY = which(is_naVnotY==1)
  n_naVnotY = length(ind_naVnotY)
  
  # NA: Y, but not V
  is_naYnotV = (1-is_naV) * is_naY
  ind_naYnotV = which(is_naYnotV==1)
  n_naYnotV = length(ind_naYnotV)
  
  # 
  Vrange = range(V,na.rm=T)
  Vmin = Vrange[1]
  Vmax = Vrange[2]
  ...
  matVtV = t(matV[-ind_naV,]) %*% matV[-ind_naV,]
  INVmatVtV = inv_cpp(matVtV)
  matMtM = t(matM[-ind_naV,]) %*% matM[-ind_naV,]
  ...
  Ky = 2
  Sy = numeric(N)
  Sy[-ind_naVorY] = kmeans(cbind(Y, M, V, Z, scaC)[-ind_naVorY,], Ky)$cluster
  Sy[ind_naVorY] = rbinom(n_naVorY,1,0.5) + 1
  Syx[,1] = Sy
  ...
  Sx = numeric(N)
  for (k in 1:Ky) {
    ind_Sy = which((Sy == k) * (is_naV == 0) == 1)
    Sx[ind_Sy] = kmeans(cbind(V, Z, scaC)[ind_Sy,], Kx)$cluster
  }
  Sx[ind_naV] = rbinom(n_naV,1,0.5) + 1
  Syx[,2] = Sx
  ...
  # update parameters for continuous posttreatment confounders
  {
    ...
    for (k in unique_Sy) {
      ...
      for (r in unique_Sx_temp) {
        ...
        Vregression_list = update_reg_con(v_temp_kr,matX_temp_kr,VbetaPars[,count],Vsig2Pars[,count],
  ...
  for (k in unique_Sy) {
    ...
    Mregression_list = update_reg_con(M[ind_Sy],matV[ind_Sy,],MbetaPars[,k],Msig2Pars[,k],
                                      a_Msig2,b_Msig2,Binv_Mbeta,aBinv_Mbeta)
    Yregression_list = update_reg_con(Y[ind_Sy],matM[ind_Sy,],YbetaPars[,k],Ysig2Pars[,k],
  ...
  # f0_v[ind_naV] = mean(f0_v[-ind_naV]) * 0
  # f0_m[ind_naV] = mean(f0_m[-ind_naV]) * 0
  # f0_y[ind_naVorY] = mean(f0_y[-ind_naVorY]) * 0
  
  # NAs -------------------------------------------------------------------------
  Y_temp = Y
  Y_temp[ind_naY] = impute_Y(Y, M, V, Z, matM, matV, matX, 
                             n_naY, ind_naY, ind_naVandY, unique_Sy, unique_Syx, Sy, Sx, 
                             YbetaPars, Ysig2Pars, MbetaPars, Msig2Pars, VbetaPars, Vsig2Pars)
  
  V_temp = V
  V_temp[ind_naV] = impute_V(Y, M, V, Z, matM, matV, matX, 
                             n_naV, ind_naV, ind_naVandY, unique_Sy, unique_Syx, Sy, Sx, 
                             YbetaPars, Ysig2Pars, MbetaPars, Msig2Pars, VbetaPars, Vsig2Pars)
  
  matV_temp = cbind(1, Z, V_temp, scaC)
  matM_temp = cbind(1, Z, Z*M, M, V_temp, scaC)
  # End NAs ---------------------------------------------------------------------
  ...
  for (gibbs_reps in 1:gibbs_total) {
    ...
    cluster_res = EDPcluster_concon(N, p_Z, p_C1, p_C2, 
                                    is_naY, is_naV, 
                                    is_naVorY, is_naVandY, is_naVnotY, 
                                    Y_temp, M, V_temp, X, matM_temp, matV_temp, matX,
                                    Sy, Sx, unique_Syx,
                                    YbetaPars, Ysig2Pars, 
                                    MbetaPars, Msig2Pars,
                                    VbetaPars, Vsig2Pars,
    ...
    for (k in unique_Sy) {
      ...
      Mregression_list = update_reg_con(M[ind_Sy],matV_temp[ind_Sy,],MbetaPars[,k],Msig2Pars[,k],
                                        a_Msig2,b_Msig2,Binv_Mbeta,aBinv_Mbeta)
      Yregression_list = update_reg_con(Y_temp[ind_Sy],matM_temp[ind_Sy,],YbetaPars[,k],Ysig2Pars[,k],
                                        a_Ysig2,b_Ysig2,Binv_Ybeta,aBinv_Ybeta)
      Msig2Pars[,k] = Mregression_list$sig2_par
      MbetaPars[,k] = Mregression_list$beta_par
      Ysig2Pars[,k] = Yregression_list$sig2_par
...
EDPmediation = function(Y, M, V, Z, C, 
                        ...
                        esttype = c("median","mean"), rho = "unif", save_cluster = FALSE){
  
  MCMCresult = EDPmediationMCMC(Y, M, V, Z, C, 
                                gibbs_iter, gibbs_burnin, gibbs_thin,
                                D, num_MC, num_MC_prior)
  
  POSTresult = EDPmediationPOST(MCMCresult, esttype, rho, save_cluster)
